...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'job_pay'),
        ...
        self.tree.column("job_pay", width=150, anchor=tk.CENTER)
        ...
        self.tree.heading("job_pay", text='Зарплата')
    ...
    # добавление данных
    def records(self, name, tel, email, job_pay):
        self.db.insert_data(name, tel, email, job_pay)
        self.view_records()

    # обновление (изменение) данных
    def update_record(self, name, tel, email, job_pay):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, job_pay=? WHERE ID=?''',
                          (name, tel, email, job_pay, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_sum = tk.Label(self, text='job_pay')
        ...
        self.entry_job_pay = ttk.Entry(self)
        self.entry_job_pay.place(x=200, y=140)
...
class Update(Child):
    ...
    def default_data(self):
        ...
        self.entry_job_pay.insert(0, row[4])
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, job_pay text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, job_pay):
        self.c.execute('''INSERT INTO db (name, tel, email, job_pay) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, job_pay))
        self.conn.commit()
